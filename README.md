...
- **BLE Scanning**: Continuous active scanning using BlueZ `SetDiscoveryFilter` (works with extended advertising / AUX PDUs)
...
In a dense RF environment (50–100+ nearby BLE advertisers), enable
`duplicate_filtering` (default `true`) so BlueZ suppresses duplicate advertisement
reports before they reach Python:
...
> **Why active-only?** The target devices (e.g. Molleau water meters) advertise
> exclusively via BLE extended advertising — the payload is in the AUX (secondary)
> PDU. Passive scanning relies on BlueZ's `AdvertisementMonitor` `or_patterns`,
> which do **not** match extended advertisements, so passive mode receives nothing.
> Active mode uses BlueZ `SetDiscoveryFilter` on the fully reassembled advertisement
> and works correctly.
...
3. **Scan Mode**: Active scanning with `duplicate_filtering` to reduce host-ward report volume
...
1. **Duplicate filtering** - BlueZ-level deduplication reduces host-ward report volume (`duplicate_filtering: true`)
